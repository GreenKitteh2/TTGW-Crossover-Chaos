		...
		if super_battle:
			if battle_node.item_pool:
				...
				if player.better_battle_rewards and current_round <= 2:
					...
					player.boost_queue.queue_text("Super Bounty!", Color.GREEN)
				else:
				...
				chest.global_position = battle_node.to_global(Vector3(2, 0, 0))
				...
				if player.better_battle_rewards and current_round <= 2:
					chest2.item_pool = ItemService.PROGRESSIVE_POOL
				else:
					chest2.item_pool = battle_node.item_pool
				chest2.override_replacement_rolls = RNG.channel(RNG.ChannelBattleChestOverrides).randf() < 0.33
				...
				s_spawned_reward_chest.emit(chest2)
				...
				chest3.override_replacement_rolls = RNG.channel(RNG.ChannelBattleChestOverrides).randf() < 0.33
				battle_node.get_parent().add_child(chest3)
				chest3.global_position = battle_node.to_global(Vector3(-2, 0, 0))
				chest3.global_rotation = battle_node.global_rotation
				s_spawned_reward_chest.emit(chest3)
		else:
			if battle_node.item_pool:
				var chest = load('res://objects/interactables/treasure_chest/treasure_chest.tscn').instantiate()
				if player.better_battle_rewards and current_round <= 2:
...
func create_v2_cog(cog: Cog) -> Cog:
	...
	if cog.skelecog:
		...
		new_cog.skelecog = true
	new_cog.overcog_chance = 0
	new_cog.overcog = cog.overcog
	new_cog.overcog_value = cog.overcog_value	
	new_cog.dna = cog.dna
...
func create_v3_cog(cog: Cog) -> Cog:
	force_watch_death.append(cog)
	var new_cog: Cog = load('res://objects/cog/cog.tscn').instantiate()
	new_cog.skelecog_chance = 0
	new_cog.exe_chance = 0
