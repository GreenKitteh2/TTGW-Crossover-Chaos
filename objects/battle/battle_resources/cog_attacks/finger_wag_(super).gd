...
const EFFECT := preload("res://objects/battle/battle_resources/status_effects/resources/status_effect_wagging.tres")

var wagging: StatusEffect
...
func action():
	...
	if hit:
		wagging = EFFECT.duplicate(true)
		wagging.target = user
		manager.add_status_effect(wagging)
