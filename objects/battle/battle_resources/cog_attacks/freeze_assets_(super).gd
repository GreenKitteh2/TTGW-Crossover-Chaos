...
const EFFECT := preload("res://objects/battle/battle_resources/status_effects/resources/status_effect_brittle.tres")
var brittle: StatusEffect
...
func action():
	...
	if hit:
		brittle = EFFECT.duplicate(true)
		brittle.target = target
		manager.add_status_effect(brittle)
