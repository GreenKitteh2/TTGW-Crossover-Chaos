extends CogAttack
const EFFECT := preload("res://objects/battle/battle_resources/status_effects/resources/status_effect_restrained.tres")
var restrained: StatusEffect
...
var hit: bool
"res://objects/battle/battle_resources/cog_attacks/cog_exclusives/restraining_order_(case_manager).tres"
	...
	throw_tween.finished.connect(
		func():
			...
			if hit:
				...
				restrained = EFFECT.duplicate(true)
				restrained.target = player
				manager.add_status_effect(restrained)
				manager.affect_target(player, damage)
