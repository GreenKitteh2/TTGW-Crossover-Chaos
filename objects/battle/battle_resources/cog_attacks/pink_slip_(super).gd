...
func pink_slip():
	...
	if hit:
		...
		manager.add_status_effect(create_debuff(target))
		manager.add_status_effect(create_debuff2(target))
...
func create_debuff(target : Player) -> StatBoost:
...
func create_debuff2(target : Player) -> StatBoost:
	var effect := STAT_BOOST.duplicate(true)
	effect.quality = StatusEffect.EffectQuality.NEGATIVE
	effect.boost = -0.25
	effect.stat = 'damage'
