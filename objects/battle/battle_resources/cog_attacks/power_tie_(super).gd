...
const EFFECT := preload("res://objects/battle/battle_resources/status_effects/resources/status_effect_awfully_embarrassed.tres")
var embarrassed: StatusEffect
...
func action() -> void:
	...
	if hit:
		...
		embarrassed = EFFECT.duplicate(true)
		embarrassed.target = player
		manager.add_status_effect(embarrassed)
