...
func action():
	...
	if hit:
		...
		if not melt_toon:
			...
			target.stats.gag_balance.Drop -= 2
			target.stats.gag_balance.Lure -= 2
			target.stats.gag_balance.Sound -= 2
			target.stats.gag_balance.Squirt -= 2
			target.stats.gag_balance.Throw -= 2
			target.stats.gag_balance.Trap -= 2
		else:
