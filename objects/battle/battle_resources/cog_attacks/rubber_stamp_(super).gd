...
const EFFECT := preload("res://objects/battle/battle_resources/status_effects/resources/status_effect_stamped.tres")
var stamped: StatusEffect
...
func action():
	...
	if hit:
		stamped = EFFECT.duplicate(true)
		stamped.target = target
		stamped.rounds = 1
		manager.add_status_effect(stamped)
