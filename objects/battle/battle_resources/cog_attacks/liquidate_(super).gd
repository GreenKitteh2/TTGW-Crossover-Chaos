...
const EFFECT := preload("res://objects/battle/battle_resources/status_effects/resources/status_effect_soaked.tres")

var soaked: StatusEffect
...
func action():
	...
	if hit:
		soaked = EFFECT.duplicate(true)
		soaked.target = target
		manager.add_status_effect(soaked)
