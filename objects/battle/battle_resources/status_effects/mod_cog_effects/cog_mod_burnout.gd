...
func apply() -> void:
	...
	damage = roundi(float(-Util.get_hazard_damage()) * 1)
...
func application_movie() -> void:
	...
	# Show move name
	movie.tween_callback(manager.show_action_name.bind("Burnout!"))
