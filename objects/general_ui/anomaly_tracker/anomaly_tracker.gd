...
const SFX_COOL := preload('res://audio/sfx/misc/MG_sfx_travel_game_bonus.ogg')
...
const SFX_AWFUL := preload('res://audio/sfx/misc/CHQ_SOS_cage_land.ogg')
...
func get_sfx(mod_type: FloorModifier.ModType) -> AudioStream:
	if is_obscured():
		return SFX_IDK
	match mod_type:
		FloorModifier.ModType.SUPERPOSITIVE:
			return SFX_COOL
