	...
	if tie_mesh:
		...
		if get_custom_texture(dna, 'custom_skelecog_tie_tex'):
			...
			match dna.department:
				...
				CogDNA.CogDept.BOARD: tie_mat.albedo_texture = load("res://models/cogs/textures/board/cog_robot_tie_board.png")
				CogDNA.CogDept.TECH: tie_mat.albedo_texture = load("res://models/cogs/textures/tech/cog_robot_tie_tech.png")
				CogDNA.CogDept.PRESS: tie_mat.albedo_texture = load("res://models/cogs/textures/press/cog_robot_tie_press.png")
