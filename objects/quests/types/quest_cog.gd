...
func randomize_objective() -> void:
	...
	if quest_type == 0:
		...
		match department:
			...
			CogDNA.CogDept.BOARD:
				cog_pool = load('res://objects/cog/presets/pools/boardbot.tres')
			CogDNA.CogDept.TECH:
				cog_pool = load('res://objects/cog/presets/pools/techbot.tres')
			CogDNA.CogDept.PRESS:
				cog_pool = load('res://objects/cog/presets/pools/pressbot.tres')	
