...
func on_battle_start(battle : BattleManager) -> void:
	for cog in battle.cogs:
		cog.stats.hp = (cog.stats.hp + (cog.stats.max_hp * 0.5))
	battle.s_participant_joined.connect(func(participant):
		if participant is Cog:
			participant.stats.hp = (participant.stats.hp + (participant.stats.max_hp * 0.5))
	)
...
func get_mod_icon() -> Texture2D:
	return load("res://ui_assets/player_ui/pause/overcharge.png")

func get_description() -> String:
	return "Cogs gain 50% extra HP"
