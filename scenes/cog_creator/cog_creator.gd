...
@onready var female_button : CheckBox = $Menus/AtrributeSelectors/VBoxContainer/GenderToggle/CheckBox
...
func _ready_attribute() -> void:
	...
	female_button.set_pressed(cog.dna.is_female_cog)
...
func female_toggled(yes: bool) -> void:
	if yes:
		cog.dna.is_female_cog = true
		cog.female = true
	else:
		cog.dna.is_female_cog = false
		cog.female = false
	cog.randomize_cog()
	_refresh_cog()
