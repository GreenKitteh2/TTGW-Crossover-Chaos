...
const MOLE_GARDEN := "res://scenes/cgc_mole_garden.tscn"
...
func enter_mole_garden() -> void:
	var gardenmusic: AudioStream = load("res://audio/music/Zen_Garden.ogg")
	SceneLoader.load_into_scene(MOLE_GARDEN)
	if not AudioManager.current_music == gardenmusic:
		AudioManager.stop_music(true)
		AudioManager.set_default_music(gardenmusic)
	hide()
	close()
